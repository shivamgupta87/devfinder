// import styles from '../styles/searchBar.module.css'
// import "../index.css";
import "../index.css"

const SearchBar = () => { 
     const btnimg = () => {
    const btn = document.querySelector(".img");
    console.log(btn);
    if (
      btn.src ==
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTl0jSxOH9WeWau8dB7awqYz1pUcKGWb1-H1w&usqp=CAU"
    ) {
      btn.src =
        "data:image/png;base64,";
    } else {
      btn.src =
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTl0jSxOH9WeWau8dB7awqYz1pUcKGWb1-H1w&usqp=CAU";
    }
  };
return <><div className="head">
            <h1>devfinder</h1>
            <h1
              style={{
                marginTop: "0",
              }}
            >
              Light{" "}
              <button className="img-btn" onClick={btnimg}>
                <img
                  className="img"
                  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTl0jSxOH9WeWau8dB7awqYz1pUcKGWb1-H1w&usqp=CAU"
                  style={{
                    backGround: "white",
                    height: "35px",
                  }}
                />
              </button>
            </h1>
          </div></>
}

export default SearchBar

